...
# To start with, we could check every possible pair. If we sum each element with every other, we're sure to either find the pair that sums to the target,
...
# (Note: you can cut the running-time significantly by returning early, and by not checking pairs more than once. 
# However, these micro-optimizations will not improve the time complexity of the solution. Do you see why?)

# Once you're done, write a comment with your solution's time complexity.


def bad_two_sum?(arr, target_sum)     # BigO time complexity : O(n**2)
    (0...arr.length).each do |i|        
        (i+1...arr.length).each do |i2|
            return true if arr[i] + arr[i2] == target_sum
        end 
    end 
    false 
end
...
# Hint: (There are a couple ways to solve this problem once it's sorted. One way involves using a very cheap algorithm that can only be used on sorted data sets.
# What are some such algorithms you know?)

def okay_two_sum?(arr, target_sum)     # BigO time complexity : O(n*logn)
    arr = arr.sort 
    arr.each_with_index do |num, i|
        match = arr.bsearch_index { |x| x <=> (target_sum - num) }
        return true if match && match != i
    end 
    false
end
...
def two_sum?(arr, target_sum)     # BigO time complexity : are we upposed to get O(n)??
    hash = {}
